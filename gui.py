...
class OutreachApp:
    ...
    def _make_header(self, title: str, subtitle: str) -> tk.Frame:
        ...
        tk.Label(inner, text=subtitle, bg=NAVY, fg="#c7d6ea", font=self.f_sub).pack(anchor="w", pady=(4, 0))
        return header

    def _make_button(self, parent, text, command, *, bg=BLUE, fg="white", height=2, compact=False):
        button = tk.Button(
            parent,
            text=text,
    ...
    def _init_style(self) -> None:
        ...
        style.configure("Field.TLabel", background=CARD, foreground=TEXT, font=self.f_label)
        style.configure("Hint.TLabel", background=CARD, foreground=MUTED, font=self.f_small)
        style.configure("TEntry", fieldbackground="#ffffff", bordercolor=BORDER, relief="flat", padding=8)
    ...
    def _build_setup(self) -> None:
        ...
        self._clear_root()
        self.root.configure(bg=BG)
        self._make_header(APP_NAME, "First-time setup")
        ...
        button = self._make_button(card, "Choose credentials.json…", self._pick_credentials)
        button.pack(fill="x", padx=22, pady=(4, 18))
    ...
    def _pick_credentials(self) -> None:
        ...
        try:
            ...
            self.setup_status.config(
                text="That file doesn't look like a Google OAuth client (credentials.json). Please pick the downloaded file.",
                fg=RED,
            )
            return
    ...
    def _build_form(self) -> None:
        ...
        self._make_header(
            ...
            "Create a personalized Gmail draft. Nothing is ever sent — you review every draft in Gmail first.",
        )
        ...
        row = 0
        tk.Frame(card, bg=CARD, height=10).grid(row=row, column=0)
        row += 1

        self.e_email = self._field(card, row, "Recipient email", self.v_email, "principal@school.org")
        row += 2
        self.e_rname = self._field(card, row, "Recipient name", self.v_rname, "e.g. Ms. Rivera")
        row += 2
        self.e_sname = self._field(card, row, "Your name (sender)", self.v_sname, "e.g. Aarav Marfatia")
        row += 2
        self.e_school = self._field(card, row, "School / Event name", self.v_school, "e.g. Sunrise Elementary School")
        row += 2

        tk.Label(card, text="Email type", bg=CARD, fg=TEXT, font=self.f_label).grid(
            row=row,
            column=0,
            sticky="w",
            padx=22,
        ...
        self.combo.grid(row=row, column=0, sticky="ew", padx=22)
        row += 1

        for var in (self.v_email, self.v_rname, self.v_sname, self.v_school, self.v_type):
            var.trace_add("write", lambda *_: self._refresh_preview())

        preview = tk.Frame(card, bg="#eef3fb", highlightbackground=BORDER, highlightthickness=1)
        preview.grid(row=row, column=0, sticky="ew", padx=22, pady=(16, 4))
        preview.columnconfigure(0, weight=1)

        tk.Label(preview, text="PREVIEW", bg="#eef3fb", fg=BLUE_DARK, font=self.f_small).grid(
            row=0, column=0, sticky="w", padx=12, pady=(8, 0)
        )
        self.lbl_desc = tk.Label(preview, text="", bg="#eef3fb", fg=TEXT, font=self.f_small, wraplength=520, justify="left")
        self.lbl_desc.grid(row=1, column=0, sticky="w", padx=12, pady=(2, 0))
        self.lbl_subject = tk.Label(preview, text="", bg="#eef3fb", fg=TEXT, font=self.f_body, wraplength=520, justify="left")
        self.lbl_subject.grid(row=2, column=0, sticky="w", padx=12, pady=(6, 0))
        self.lbl_attach = tk.Label(preview, text="", bg="#eef3fb", fg=MUTED, font=self.f_small, wraplength=520, justify="left")
        self.lbl_attach.grid(row=3, column=0, sticky="w", padx=12, pady=(4, 0))
        self.lbl_cc = tk.Label(preview, text="", bg="#eef3fb", fg=MUTED, font=self.f_small, wraplength=520, justify="left")
        self.lbl_cc.grid(row=4, column=0, sticky="w", padx=12, pady=(2, 10))
        row += 1

        actions = tk.Frame(card, bg=CARD)
        actions.grid(row=row, column=0, sticky="ew", padx=22, pady=(14, 6))
        actions.columnconfigure(0, weight=1)
        actions.columnconfigure(1, weight=0)

        self.btn_create = self._make_button(actions, "Create Draft", self._on_create)
        ...
        self.status = tk.Label(card, text="", bg=CARD, fg=MUTED, font=self.f_small, wraplength=560, justify="left")
        self.status.grid(row=row, column=0, sticky="w", padx=22, pady=(2, 14))
        row += 1

        footer = tk.Frame(self.root, bg=BG)
        footer.pack(fill="x", padx=24, pady=(0, 14))
        cc_text = "Every draft is CC'd to: " + ", ".join(od.CC_RECIPIENTS)
        tk.Label(footer, text=cc_text, bg=BG, fg=MUTED, font=self.f_small, wraplength=580, justify="left").pack(anchor="w")
        tk.Label(footer, text="Shared CircuitRunners account · drafts only, never sends", bg=BG, fg=MUTED, font=self.f_small).pack(anchor="w")

        self.root.bind("<Return>", lambda event: self._on_create())
    ...
    def _field(self, parent, row, label, var, hint):
        ...
        entry.grid(row=row + 1, column=0, sticky="ew", padx=22, ipady=7)
    ...
    def _refresh_preview(self, *_args):
        ...
        if template:
            ...
            if attachments:
                self.lbl_attach.config(text=f"Attaches {len(attachments)} photo(s): " + ", ".join(attachments))
            else:
    ...
    def _on_clear(self):
        for entry, var in (
            (self.e_email, self.v_email),
            (self.e_rname, self.v_rname),
    ...
    def _on_create(self):
        ...
        if missing:
            label = ", ".join(part.replace("_", " ").lower() for part in missing)
            self._set_status(f"Please fill in: {label}.", "error")
            return

        if row["Template_Type"] not in od.TEMPLATES:
            self._set_status("Pick a valid email type.", "error")
            return
    ...
    def _worker(self, row):
        try:
            ...
            paths = [os.path.join(od.ATTACHMENTS_DIR, name) for name in template["attachments"]]
            ...
            self.q.put(("error", "Couldn't sign in to Gmail. Check the app's credentials.", str(exc)))
        except FileNotFoundError as exc:
            self.q.put(("error", "A demo photo is missing from the app.", str(exc)))
        except Exception as exc:  # noqa: BLE001
            self.q.put(("error", "Something went wrong creating the draft.", str(exc)))

    def _poll(self):
        try:
            kind, left, right = self.q.get_nowait()
        except queue.Empty:
            self.root.after(100, self._poll)
            return
        ...
        if kind == "ok":
            self._set_status(
                f"✓ Draft created for {left} ({right}). Open Gmail → Drafts to review and send.",
                "ok",
            )
        else:
            self._set_status(f"✕ {left}", "error")
